...
# TEST OPEN SINGLE FILE HANDLE INSTEAD OF CONSTANTLY REOPENING
def write_remap_bam(bam_file, intersect_file, r1_out, r2_out, samples, threads=0, max_seqs=64):
    ...
    with AlignmentFile(bam_file, "rb", threads=threads+1) as bam, tempfile.TemporaryDirectory() as tmpdir:
        ...
        if len(samples) > 1:
            for chrom in remap_chroms:
                swap_chrom_alleles_multi(bam=bam, out_dir=tmpdir,
            ...
            for chrom in remap_chroms:
                ...
                swap_chrom_alleles(bam=bam, out_dir=tmpdir,
                                   df=intersect_df, chrom=chrom,
                                   read_stats=read_stats,
                                   threads=threads)
...
def swap_chrom_alleles(bam, out_dir, df, chrom, read_stats, threads=0):
    ...
    with AlignmentFile(out_bam, "wb", header=bam.header, threads=threads+1) as out_file:
    ...
    collate_cmd = ["samtools", "collate",
                   "-@", str(threads), "-u", "-O", out_bam]
    
    fastq_cmd = ["samtools", "fastq",
                 "-@", str(threads), "-1", r1_out, "-2", r2_out]
...
def swap_chrom_alleles_multi(bam, out_dir, df, chrom, read_stats, threads=0):
    
    # column data
    df_cols = df.columns[:5]
    # hap_cols = df.columns[5:]
    hap_cols = pl.Series(df.columns[5:]) # try this version without transpose 
    ...
    with AlignmentFile(out_bam, "wb", header=bam.header, threads=threads+1) as out_file:
        ...
        for read1, read2 in paired_read_gen_stat(bam, read_stats, chrom=chrom):
            ...
            arr, uniq_idx = np.unique(read_df.select(pl.col(hap_cols).str.concat("")).row(0), return_index=True)
            unique_cols = hap_cols[uniq_idx]
