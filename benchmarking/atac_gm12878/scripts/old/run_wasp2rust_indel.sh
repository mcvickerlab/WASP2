#!/bin/bash
# WASP2-Rust (Unified Pipeline) benchmark on GM12878 ATAC-seq data - WITH INDELs
# Uses Rust unified_make_reads_parallel with include_indels=True
#
# For Figure 1B (timing) and Figure 1C (pre/post remap read counts)
# NOTE: Figure 1C for this run focuses ONLY on INDEL-overlapping reads
#
#$ -N wasp2rust_gm12878_indel
#$ -V
#$ -pe iblm 8
#$ -l h_vmem=32G
#$ -j y
...
OUTPUT_DIR="${BENCHMARK_DIR}/results/wasp2rust_indel_${TIMESTAMP}"
mkdir -p "${OUTPUT_DIR}"
...
echo "WASP2-Rust INDEL Unified Pipeline Benchmark - GM12878 ATAC-seq" > ${PROFILE_LOG}
echo "================================================================" >> ${PROFILE_LOG}
...
echo "========================================"
echo "WASP2-Rust INDEL Benchmark on GM12878 ATAC-seq"
...
# Extract variant BED files for Figure 1C analysis
echo "Creating variant BED files for read counting..."

# Create both SNP and INDEL BED files for completeness
# SNPs: ref and alt both single base
bcftools query -f '%CHROM\t%POS0\t%POS\t%REF\t%ALT\n' ${VCF} | \
    awk '$4 !~ /,/ && $5 !~ /,/ && length($4)==1 && length($5)==1' > ${OUTPUT_DIR}/snps.bed
SNP_COUNT=$(wc -l < ${OUTPUT_DIR}/snps.bed)
echo "SNP variants in BED: ${SNP_COUNT}"

# INDELs: ref and alt different lengths (insertions or deletions)
bcftools query -f '%CHROM\t%POS0\t%POS\t%REF\t%ALT\n' ${VCF} | \
    awk '$4 !~ /,/ && $5 !~ /,/ && length($4)!=length($5)' > ${OUTPUT_DIR}/indels.bed
INDEL_COUNT=$(wc -l < ${OUTPUT_DIR}/indels.bed)
echo "INDEL variants in BED: ${INDEL_COUNT}"

# Combined for pipeline (but Figure 1C focuses on INDEL-overlapping)
cat ${OUTPUT_DIR}/snps.bed ${OUTPUT_DIR}/indels.bed | sort -k1,1 -k2,2n > ${OUTPUT_DIR}/variants.bed
TOTAL_VARIANTS=$(wc -l < ${OUTPUT_DIR}/variants.bed)
echo "Total variants in BED: ${TOTAL_VARIANTS}"

# Count reads overlapping INDELs in original BAM (for Figure 1C pre-remap)
echo ""
echo "Counting reads overlapping INDELs in original BAM..."
INDEL_READS_ORIGINAL=$(${BEDTOOLS} intersect -a ${ORIGINAL_BAM} -b ${OUTPUT_DIR}/indels.bed -u | \
    ${SAMTOOLS} view | cut -f1 | sort -u | wc -l)
echo "Original read pairs overlapping INDELs: ${INDEL_READS_ORIGINAL}"

# Count reads overlapping SNVs in original BAM (for Figure 1C pre-remap)
echo "Counting reads overlapping SNVs in original BAM..."
SNV_READS_PRE=$(${BEDTOOLS} intersect -a ${ORIGINAL_BAM} -b ${OUTPUT_DIR}/snps.bed -u | \
    ${SAMTOOLS} view | cut -f1 | sort -u | wc -l)
echo "Original read pairs overlapping SNVs: ${SNV_READS_PRE}"
...
# -----------------------------------------------------------------------------
# STEP 1: WASP2-Rust unified make-reads (WITH INDELs)
# -----------------------------------------------------------------------------
echo "========================================"
echo "STEP 1: WASP2-Rust unified make-reads (WITH INDELs)"
echo "========================================"
echo "" >> ${PROFILE_LOG}
echo "STEP 1: WASP2-Rust unified make-reads (WITH INDELs)" >> ${PROFILE_LOG}
...
# -----------------------------------------------------------------------------
# POST-BENCHMARK: Count variant-overlapping reads in kept BAM (for Figure 1C)
# -----------------------------------------------------------------------------
echo ""
echo "========================================"
echo "FIGURE 1C: Counting variant-overlapping reads"
echo "========================================"

# Count INDEL-overlapping reads post-WASP
echo "Counting reads overlapping INDELs in kept BAM..."
INDEL_READS_KEPT=$(${BEDTOOLS} intersect -a ${OUTPUT_DIR}/remap_keep.bam -b ${OUTPUT_DIR}/indels.bed -u | \
    ${SAMTOOLS} view | cut -f1 | sort -u | wc -l)
echo "Kept read pairs overlapping INDELs: ${INDEL_READS_KEPT}"

# Calculate INDEL pass rate
if [ ${INDEL_READS_ORIGINAL} -gt 0 ]; then
    INDEL_PASS_RATE=$(echo "scale=2; ${INDEL_READS_KEPT} * 100 / ${INDEL_READS_ORIGINAL}" | bc)
else
    INDEL_PASS_RATE=0
fi
echo "INDEL read pass rate: ${INDEL_PASS_RATE}%"

# Count SNV-overlapping reads post-WASP
echo "Counting reads overlapping SNVs in kept BAM..."
SNV_READS_POST=$(${BEDTOOLS} intersect -a ${OUTPUT_DIR}/remap_keep.bam -b ${OUTPUT_DIR}/snps.bed -u | \
    ${SAMTOOLS} view | cut -f1 | sort -u | wc -l)
echo "Kept read pairs overlapping SNVs: ${SNV_READS_POST}"
...
# Count total kept reads (for general stats)
...
echo ""
echo "Figure 1C - Variant-overlapping reads:"
echo "  SNV-overlapping:"
echo "    Pre-WASP:   ${SNV_READS_PRE} read pairs"
echo "    Post-WASP:  ${SNV_READS_POST} read pairs"
echo "    Pass rate:  ${SNV_PASS_RATE}%"
echo "  INDEL-overlapping:"
echo "    Pre-WASP:   ${INDEL_READS_ORIGINAL} read pairs"
echo "    Post-WASP:  ${INDEL_READS_KEPT} read pairs"
echo "    Pass rate:  ${INDEL_PASS_RATE}%"
echo ""
...
cat > ${OUTPUT_DIR}/benchmark_results.json << EOF
{
    ...
    "indel_reads_pre": ${INDEL_READS_ORIGINAL},
    "indel_reads_post": ${INDEL_READS_KEPT},
    "indel_pass_rate_percent": ${INDEL_PASS_RATE}
}
EOF
...
echo ""
echo "Key output files for Figure 1C (INDEL-overlapping):"
echo "  - original.bam (pre-remap)"
echo "  - remap_keep.bam (post-remap)"
echo "  - indels.bed (INDEL positions only)"
echo "  - snps.bed (SNP positions for reference)"
