...
# FIXED: Uses ONLY heterozygous SNPs (as per WASP documentation)
...
OUTPUT_DIR="${BENCHMARK_DIR}/results/wasp1_snp_FIXED_${TIMESTAMP}"
mkdir -p "${OUTPUT_DIR}"
...
echo "WASP1 SNP-only Pipeline Benchmark (FIXED - het-only) - GM12878 ATAC-seq" > ${PROFILE_LOG}
echo "=======================================================================" >> ${PROFILE_LOG}
...
echo "========================================"
echo "WASP1 SNP FIXED Benchmark on GM12878 ATAC-seq"
echo "Timestamp: ${TIMESTAMP}"
echo "========================================"
echo "FIXED: Using ONLY heterozygous SNPs (as per WASP documentation)"
...
# -----------------------------------------------------------------------------
# PRE-BENCHMARK: Filter VCF to HETEROZYGOUS SNPs ONLY (CRITICAL FIX!)
# -----------------------------------------------------------------------------
echo "========================================"
echo "PRE-BENCHMARK: Filter VCF to het-only SNPs"
echo "========================================"
echo "WASP documentation states: 'SNP files should contain ONLY heterozygous sites'"
...
# Filter for:
# 1. Heterozygous genotypes only (0/1, 1/0, 0|1, 1|0)
# 2. SNPs only (not indels)
...
echo "  Filtered out $(( ALL_SNPS - HET_SNPS )) homozygous SNPs"

# -----------------------------------------------------------------------------
# PRE-BENCHMARK: snp2h5 VCF -> HDF5 conversion (using het-only VCF)
# -----------------------------------------------------------------------------
echo "========================================"
...
# Split het-only VCF by chromosome for snp2h5
echo "Splitting het-only VCF by chromosome..."
for i in {1..22}; do
    echo "  Processing chr${i}..."
    ${BCFTOOLS} view -r "chr${i}" ${HET_VCF} -Oz -o ${SNP2H5_DIR}/chr${i}.vcf.gz 2>/dev/null || \
    ${BCFTOOLS} view -r "${i}" ${HET_VCF} -Oz -o ${SNP2H5_DIR}/chr${i}.vcf.gz 2>/dev/null || true
done
...
# Extract variant BED for Figure 1C analysis (het-only)
echo "Creating variant BED file for read counting (het-only)..."
...
# Count keep / remap_keep reads separately for parity with Figure 1C
KEEP_READS=$(${SAMTOOLS} view -c original.keep.bam)
REMAP_KEEP_READS=$(${SAMTOOLS} view -c remap_filtered.bam)
FINAL_READS=$(( KEEP_READS + REMAP_KEEP_READS ))
...
# Compute overall pass rate using final reads (keep + remap_keep)
...
echo "========================================"
echo "BENCHMARK RESULTS (FIXED - het-only)"
echo "========================================"
echo ""
...
echo "  Original reads:  ${ORIGINAL_READS}"
echo "  Keep (no SNV):   ${KEEP_READS}"
echo "  Remap-keep (SNV):${REMAP_KEEP_READS}"
...
echo ""
echo "Variant counts:"
echo "  Het-only SNPs used: ${VARIANT_COUNT}"
echo ""
...
echo "  - variants.bed (het-only SNP positions)"
