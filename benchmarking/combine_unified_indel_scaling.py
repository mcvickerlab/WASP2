#!/usr/bin/env python3
"""
Combine per-task unified INDEL scaling TSVs into a single COMBINED.tsv.

Each array task in `benchmark_unified_scaling_indels.sh` writes a TSV to:
  benchmarking/results/unified_indel_scaling_<timestamp>/wasp2_unified_indel_scaling.tsv
...
def main() -> None:
    ...
    parser.add_argument(
        "--pattern",
        default="benchmarking/results/unified_indel_scaling_*/wasp2_unified_indel_scaling.tsv",
        help="Glob pattern for per-task TSVs.",
    )
    parser.add_argument(
        "--out",
        default="benchmarking/results/wasp2_unified_indel_scaling_COMBINED.tsv",
